(function () {
    ...
    class DinoUI {
        ...
        genSections() {
            ...
            <div style="margin:25px 0 8px 0;border-bottom:1px solid var(--soft);padding-bottom:10px;">
            <h3 style="color:var(--accent);margin:0 0 8px 0;font-size:18px;">Sprite Manager <span style="font-size:11px;">(chromedino.com only)</span></h3>
            <div class="dc-row" style="gap:10px;flex-wrap:wrap;">
            <span class="dc-label" title="URL of sprite sheet image">Sprite Sheet URL</span>
            ...
            <div class="dc-row" style="gap:10px;flex-wrap:wrap;">
            Presets:
            ${SPRITE_PRESETS.map(s=>`<button class="preset-sprite dc-btn-action" data-url="${s.url}">${s.name}</button>`).join('')}
            </div>
            </div>
            </div>
